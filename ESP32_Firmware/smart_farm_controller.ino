...
void loop() {
  ...
  if (Firebase.ready()) {
    ...
    // Read from Firebase
    if (Firebase.RTDB.getString(&fbdo, "gsheetdata/motor")) {
      String motorStatusStr = fbdo.stringData();
      Serial.print("Motor Status from Firebase (string): ");
      Serial.println(motorStatusStr);

      // Remove escaped quotes
      motorStatusStr.replace("\"", "");
